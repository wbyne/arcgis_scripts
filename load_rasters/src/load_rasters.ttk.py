...
#class ToolClass10(object):
    ...
    def __init__(self, parent):
        ...
        self.root.title("Raster Finder - BETA BUILD 14 Nov 2013")
...
#        self.aer_96_path="\\\\augnas005\\ortho\\BlackWhite1997Orthos\\"
        ...
        self.aer_11_path="c:\\mnt\\gis\\raster\\ortho\\GARICH11_4Inch\\"
        self.LIDAR_path="c:\\mnt\\gis\\raster\\LAS_1.2\\ArcGrid\\"
        self.rec_aud_path_map="C:\\mnt\project_archives\\rectified\\aud\\"
        self.rec_aed_path_map="C:\\mnt\project_archives\\rectified\\aed\\"            
        self.rec_aud_path_paper="C:\\mnt\project_archives\\prints\\aud\\"
        self.rec_aed_path_paper="C:\\mnt\project_archives\\prints\\aed\\"            
        
        
        btnSettings = Button(self.frameSave, text="Settings", command=self.setSettings)
        btnSettings.grid(row=0,column=0)
        btnSettings.config(state=DISABLED)
...
#        btn.pack()
        ...
        lblSet6 = Label(otherFrame, text="prints aud:")
        lblSet7 = Label(otherFrame, text="prints aed:")
...
#        arcpy.MakeTableView_management("ibounds",mytable2)
        arcpy.MakeTableView_management("RecordDrawings",mytable2)
        myfields2=arcpy.ListFields(mytable2)
        mytable3 = "c:/temp/resultsview3"
        arcpy.MakeTableView_management("Ga_East_Final_Tiles",mytable3)
        myfields3=arcpy.ListFields(mytable3)
        mytable4 = "c:/temp/resultsview4"
        arcpy.MakeTableView_management("Ortho_GARICH_2011_4Inch",mytable4)
        myfields4=arcpy.ListFields(mytable4)
        ...
        self.listBoxResults.insert(END, "----------LIDAR-----------")
        self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("LIDAR","LIDAR","2011","Null","Null",0,0))
        myfields3 = ["Name"]
        with arcpy.da.SearchCursor(mytable3,myfields3) as cursor:
                 for row in cursor:
                     self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("LIDAR","GRID","2011","Null",row[0],0,0))
                     self.listBoxDetails.insert(END, row[0])
                     self.listBoxResults.insert(END, row[0])

        self.listBoxResults.insert(END, "--------AERIALS_2011---------")
        self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("Aerials","Flyover","2011","Null","Null",0,0))
        myfields4 = ["Name"]
        with arcpy.da.SearchCursor(mytable4,myfields4) as cursor:
                 for row in cursor:
                     self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("AERIALS","FLYOVER","2011","Null",row[0],0,0))
                     self.listBoxDetails.insert(END, row[0])
                     self.listBoxResults.insert(END, row[0])
...
#        localResult = self.ListRes[index]
        ...
        if mysplit[0]=="Aerials" or mysplit[0] == "Scans" or mysplit[0] == "LIDAR" or mysplit[0] == "Aerials2011":
...
#####################LIDAR####################
        elif mysplit[0] == "LIDAR":
#        self.LIDAR_path="c:\\mnt\\gis\\raster\\LAS_1.2\\ArcGrid\\"            
            tsplit = mysplit[4]
            print "LIDAR: "+ mysplit[4]
            path=self.LIDAR_path
            path=path+mysplit[4]
            print path
            print "End LIDAR Parsing"

#####################Aerials 2011####################
        elif mysplit[0] == "Aerials2011":            
#        self.aer_11_path="c:\\mnt\\gis\\raster\\ortho\\GARICH11_4Inch\\"
            tsplit = mysplit[4]
            print "Aerials2011: "+mysplit[4]+".tif"
            path=self.aer_11_path
            path=path+mysplit[4]+".tif"
            print path
            print "End Aerial2011 Parsing"
...
#####################Record Drawings####################
        ...
        #if os.path.isdir(path): #LIDAR
         #   if (mysplit[0] == "LIDAR"):
          #      tkMessageBox.showinfo("Please Check Your Number and Try Again","Arc/Info Grids Don\'t Seem to Have a Paper Viewer.  Sorry")
           #     return 0
            
        if mysplit[0] == "Aerials": ###no, don't need to strip the last two digits or mysplit[0] == "Aerials2011":
            weslyr=str(tsplit) +":"+ str(mysplit[2]) #tsplit[:-2]
        elif mysplit[0] == "LIDAR":
            weslyr=tsplit
        elif mysplit[0] == "Aerials201":
            weslyr=tsplit
        else:
            weslyr=tsplit[:-4]
...
#####################Record Drawings####################
            ...
            if os.path.isfile(path+".sid"):
                ...
                path=pathaed+".jpg" #need to check for img and jp2 formats....
                print path + "oh jpg"
            elif os.path.isfile(path+".pdf"):
                path=path+".pdf" #need to check for img and jp2 formats....
                print path + "oh pdf"
            elif os.path.isfile(pathaed+".pdf"):
                path=pathaed+".pdf" #need to check for img and jp2 formats....
                print path + "oh pdf"                
            else:
                tkMessageBox.showinfo("Well that seems to be it","That file doesn't exist")
...
#            args1 = ("cmd /c start"+"|"+"title"+"|"+path) #also works, but only for jpgs/tifs, NOT sids, WHEN launched from ArcGIS.....typical.
            print "here 1"
            if (mysplit[0]=="Aerials"): # or mysplit[0]=="Aerials2011"):
                if (mysplit[2]=="2007"):
...
#        arcpy.SelectLayerByAttribute_management("ibounds", "CLEAR_SELECTION")
        arcpy.SelectLayerByAttribute_management("RecordDrawings", "CLEAR_SELECTION")
        arcpy.SelectLayerByAttribute_management("Ga_East_Final_Tiles","CLEAR_SELECTION")
        arcpy.SelectLayerByAttribute_management("Ortho_GARICH_2011_4Inch","CLEAR_SELECTION")
...
##        arcpy.SelectLayerByLocation_management("ibounds", "INTERSECT", thepoly, "", "ADD_TO_SELECTION")
        arcpy.SelectLayerByLocation_management("RecordDrawings", "INTERSECT", thepoly, "", "ADD_TO_SELECTION")
        arcpy.SelectLayerByLocation_management("Ga_East_Final_Tiles", "INTERSECT", thepoly, "" , "ADD_TO_SELECTION")
        arcpy.SelectLayerByLocation_management("Ortho_GARICH_2011_4Inch", "INTERSECT",thepoly, "", "ADD_TO_SELECTION")
...
#                     self.listBoxDetails.insert(END, row[1])
        ...
        self.listBoxResults.insert(END, "-----LIDAR-----")
        self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("LIDAR","ArcGrid","2011","Null","Null",0,0))
        myfields3 = ["Name"]
        with arcpy.da.SearchCursor(mytable3,myfields3) as cursor:
                 for row in cursor:
                     self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("LIDAR","ArcGrid","2011","Null",row[0],0,0))
                     self.listBoxDetails.insert(END, row[0])
                     self.listBoxResults.insert(END, row[0])

        self.listBoxResults.insert(END, "-----2011 Aerials 4 Inch-----")
        self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("Aerials2011","Flyover","2011","Null","Null",0,0))
        myfields4 = ["Name"]
        with arcpy.da.SearchCursor(mytable4,myfields4) as cursor:
                 for row in cursor:
                     self.ListRes.append("{0}|{1}|{2}|{3}|{4}|{5}|{6}".format("Aerials2011","Flyover","2011","Null",row[0],0,0))
                     self.listBoxDetails.insert(END, row[0])
                     self.listBoxResults.insert(END, row[0])
#        mylist = ["1","2","3","4"]
#        for row in mylist: #ListRes: #python book p 154 @ top, list iteration
...
#        arcpy.SelectLayerByLocation_management("ibounds", "CONTAINS", pointGeom, "%d INCHES" % searchdistance, "ADD_TO_SELECTION")
        arcpy.SelectLayerByLocation_management("RecordDrawings", "CONTAINS", pointGeom, "%d INCHES" % searchdistance, "ADD_TO_SELECTION")
...
#### it lowered the results found form 571 to 8, which seems more appropriate.  It may be the switch to Contains, or the distance.
        ...
        #populate_found()
#        return
        arcpy.SelectLayerByLocation_management("Ga_East_Final_Tiles", "CONTAINS", pointGeom, "%d INCHES" % searchdistance, "ADD_TO_SELECTION")
        arcpy.SelectLayerByLocation_management("Ortho_GARICH_2011_4Inch", "CONTAINS", pointGeom, "%d INCHES" % searchdistance, "ADD_TO_SELECTION")
